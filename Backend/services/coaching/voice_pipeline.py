...
class VoicePipeline:
    ...
    def process_event(self, event, exercise, metrics):
        ...
        if voice:
            # Pad the audio with a random byte so Streamlit treats repeated feedback as a new audio file
            voice += bytes([random.randint(0, 255)])
...
def autoplay_audio(audio_bytes):
    ...
    # Hide the audio player using absolute positioning instead of display:none to bypass Chrome's autoplay blockers
    st.markdown("<style>[data-testid='stAudio'] { position: absolute; left: -9999px; width: 1px; height: 1px; }</style>", unsafe_allow_html=True)
